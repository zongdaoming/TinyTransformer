...
from spring.nart.tools.pytorch.match_chain import _scope_module_name
from spring.nart.tools.pytorch.trace_graph import trace_graph
import logging
from spring.nart.utils import deprecated
...
def convert_v2(model, input_shapes, filename="test", log=True,
    ...
    '''
    custom_info = {"Method": "convert", "Module": "spring.nart.tools"}
    try:
        module_names = export_onnx(model, input_shapes, filename, input_names=input_names, output_names=output_names,
                log=False, input_dict=input_dict, verbose=verbose, cloze=cloze)
        convert_onnx_v2(filename + '.onnx', log=False, input_names=input_names,
...
from spring.nart.utils.alter.caffe import NonParamLayer
...
def convert_onnx_v2(filename, log=True, input_names=None, output_names=None,
                                verbose=False, output_weights=True, module_names=None):
    if verbose:
        print("====convert: start of onnx-to-caffe export")

    custom_info = {"Method": "convert_onnx", "Module": "spring.nart.tools"}
    try:
        from spring.nart.tools.pytorch.network_utils import merge_shuffle_channel_nodes, merge_reshape_nodes
        model = load(filename)
        ...
        from spring.nart.utils.alter import CaffeAlter, CaffeAlterContext
        from spring.nart.utils.alter.caffe import register_caffe_layer, Layer
        from spring.nart.core import Graph, Node
        from spring.nart.utils.onnx_utils import OnnxDecoder
        from spring.nart.passes import ConstantToInitializer, FoldConstantToAttr, DeadCodeElimination, ExtractEltwise, EliminateNodes
        from spring.nart.utils.passes import ExtractHswish, ExtractHsigmoid, ExtractGemm, EliminateReshapeNode
        from spring.nart.proto import nart_caffe_pb2 as caffe_pb2
        from spring.nart.ops.op import Dropout, Cast
...
def convert_v1(model, input_shapes, filename="test", log=True,
            input_names=None, output_names=None, input_dict=None,
            verbose=False, output_weights=True, cloze=True):
    '''
    Convert a pytorch model into a caffe model
    ...
    custom_info = {"Method": "convert", "Module": "spring.nart.tools"}
    try:
        module_names = export_onnx(model, input_shapes, filename, log=False,
                    input_dict=input_dict, verbose=verbose, cloze=cloze)
        convert_onnx_v1(filename + '.onnx', log=False, input_names=input_names,
                    output_names=output_names, verbose=verbose, output_weights=output_weights,
                    module_names=module_names)
        custom_info["Status"] = "Success"
        custom_info["Caffe Model Size"] = '{} Bytes'.format(getsize(filename + '.caffemodel'))
...
def convert_onnx_by_return(model, input_shapes, filename="test", log=True,
    ...
    custom_info = {"Method": "export_onnx", "Module": "spring.nart.tools"}
    try:
        model.cpu()
        model.eval()
        ...
        if input_dict is not None:
            ...
            export(model, (input_dict,), filename + ".onnx", verbose=verbose, enable_onnx_checker=False)
        else:
            export(model, input_variables, filename + ".onnx", verbose=verbose, enable_onnx_checker=False)
        custom_info["Status"] = "Success"
        custom_info["ONNX Model Size"] = '{} Bytes'.format(getsize(filename + '.onnx'))
    except Exception as e:
        custom_info["Status"] = str(e)
...
def export_onnx(model, input_shapes, filename, input_names=None, output_names=None, log=True,
                input_dict=None, verbose=False, cloze=True):
    '''
    Convert a pytorch model into an onnx model
    @params:
    ...
    try:
        ...
        if input_dict is not None:
            input_dict['image'] = input_variables
            # export(model, (input_dict,), filename + ".onnx", verbose=verbose, enable_onnx_checker=False)
            export(model, (input_dict,), filename + ".onnx", verbose=verbose, enable_onnx_checker=True)
            # reset input_variables cause `torch.onnx.export` side-effect
            input_dict['image'] = input_variables
            graph = trace_graph(model, (input_dict,), torch.onnx.OperatorExportTypes.ONNX)
        else:
            export(model, input_variables, filename + ".onnx", verbose=verbose, enable_onnx_checker=True)
            graph = trace_graph(model, input_variables, torch.onnx.OperatorExportTypes.ONNX)
        ...
        module_names = list(map(
            lambda x: _scope_module_name(x.scopeName()),
            graph.nodes()
            ))
...
def convert_onnx_v1(filename, log=True, input_names=None, output_names=None,
                 verbose=False, output_weights=True, module_names=None):
    '''
...
def rearrange_channel(filename, indices, log=True):
    ...
    if log:
        custom_info = {"Method": "rearrange_channel", "Module": "spring.nart.tools"}
        log_thread = threading.Thread(target=send, args=(custom_info,))
        log_thread.start()
