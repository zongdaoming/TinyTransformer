...
class Attention(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        k = k.permute(0, 2, 1, 3)
        v = v.permute(0, 2, 1, 3)
        pre_attn  = q @ k.transpose(-2, -1) * self.scale
        print(f"q.shaqpe {q.shape}, k.shape {k.shape}, v.shape{v.shape}")
        print(f"pre_attention shape {pre_attn.shape}")
        attn = (
...
class AttentionSubsample(torch.nn.Module):
    def __init__(self,
        ...
        points = list(itertools.product(range(self.row), range(self.row)))
...
class TransformerEncoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        for i, (ed, kd, dpth, nh, ar, mr, do) in enumerate(
            ...
            if do[0] == 'Subsample':
                resolution_ = [math.ceil(x - 1)/do[5] + 1 for x in resolution]
                self.blocks.append(
                    AttentionSubsample(
                        *embed_dim[i:i + 2],
                        key_dim=do[1],
