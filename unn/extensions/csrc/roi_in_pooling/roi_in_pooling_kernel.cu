...
__global__ void ROIInPoolForward(
    const int nthreads, 
    ...
    CUDA_KERNEL_LOOP(index, nthreads)
    {
        ...
        for (int h = hstart; h < hend; ++h) {
            for (int w = wstart; w < wend; ++w) {
                ...
                scalar_t now_data = -FLT_MAX;
                if (h >= roi1_start_h && h <= roi1_end_h && w >= roi1_start_w && w <= roi1_end_w) now_data = bottom_data[bottom_data_offset + bottom_index];
                if (h >= roi2_start_h && h <= roi2_end_h && w >= roi2_start_w && w <= roi2_end_w) now_data = bottom_data[bottom_data_offset + bottom_index];
    ...
    Tensor top_data, Tensor argmax_data)
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(bottom_data.type(), "roi_in_pooling_forward_cuda", ([&] {
        ROIInPoolForward<scalar_t><<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock>>>(
          output_size, 
...
template <typename scalar_t>
__global__ void ROIInPoolBackward(const int nthreads, const scalar_t* top_diff,
...
int ROIInPoolBackwardLaucher(Tensor top_diff, const float spatial_scale, const int batch_size, const int num_rois,
    ...
    Tensor bottom_diff, Tensor argmax_data)
{
    ...
    cudaError_t err;
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(top_diff.type(), "roi_in_pooling_backward_cuda", ([&] {
        ROIInPoolBackward<scalar_t><<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock>>>(
          output_size, 
