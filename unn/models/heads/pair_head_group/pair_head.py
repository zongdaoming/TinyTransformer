...
class PairWiseNet(nn.Module):
    ...
    def get_loss(self, input):
        ...
        assert len(np.unique(inds)) == len(pair_rois)
        ...
        if isinstance(pred_rcnn_cls, list):
            ...
            for i, pred_cls in enumerate(pred_rcnn_cls):
                cur_loss, cur_acc = self.cal_group_loss(input, tmp_rois1, tmp_rois2, tmp_pair_rois, pred_cls, rcnn_gt_cls, 
                        roiTable, str(i) + '_', self.cfg.get("cls_loss_scale", 1.0), self.binary_mask)
                loss.update(cur_loss)
                acc.update(cur_acc)
        else:
            loss, acc = self.cal_group_loss(input, tmp_rois1, tmp_rois2, tmp_pair_rois, pred_rcnn_cls, rcnn_gt_cls, 
                    roiTable, "", self.cfg.get("cls_loss_scale", 1.0), self.binary_mask)
        predict_vector = None
        predict_target = None
    ...
    def make_matrix(self, x, gt, rois1, rois2):
       pass 

    def get_matrix_loss(self, dt, gt):
        idx = torch.nonzero(gt).squeeze()
        try:
            ...
            return None
        if len(idx) == 1:
            return F.cross_entropy(dt.view(1,-1), idx[0].view(1))
        else:
            gt_id = idx[0]
            loss_idx = [x for x in range(len(dt)) if x != idx[1]]
            loss1 = F.cross_entropy(dt[loss_idx].view(1,-1), gt_id.view(1))
            gt_id = idx[1]
            loss_idx = [x for x in range(len(dt)) if x != idx[0]]
            loss2 = F.cross_entropy(dt[loss_idx].view(1, -1), (gt_id - 1).view(1))
            return (loss1 + loss2) / 2

    def cal_group_loss(self, input, rois1, rois2, pair_idx,  pred_rcnn_cls, rcnn_gt_cls, roiTable, 
            prefix, cls_loss_scale, binary_mask=None):
        roiTable = self.create_table(pair_idx)
        loss = {}
        def f2(x):
        ...
        # rcnn_cls: N * 1
        # make matrix
        B = len(input['image_info'])
        losses = []
        for b_ix in range(B):
            idx = np.where(pair_idx[:, 0] == b_ix)[0]
            tmp_pair_idx = pair_idx[idx]
            body_idx = np.unique(tmp_pair_idx[:, 1])
            face_idx = np.unique(pair_idx[idx[rois2[idx][:, -1] == 2], 2])
            hand_idx = np.unique(pair_idx[idx[rois2[idx][:, -1] == 3], 2])
            ...
            if len(order_idx) == 0:
                continue
            dt_matrix = pred_rcnn_cls[order_idx]
            dt_matrix = dt_matrix.view(len(body_idx), len(hand_idx))
            gt_matrix = rcnn_gt_cls[order_idx]
            gt_matrix = gt_matrix.view(len(body_idx), len(hand_idx)).half()
            #pdb.set_trace()

            for x in range(gt_matrix.shape[0]):
                loss_row = self.get_matrix_loss(dt_matrix[x], gt_matrix[x]) 
                if loss_row is not None:
                    losses.append(loss_row)
            for x in range(gt_matrix.shape[1]):
                loss_col = self.get_matrix_loss(dt_matrix[:, x], gt_matrix[:, x]) 
            ...
            if len(order_idx) == 0:
                continue
            dt_matrix = pred_rcnn_cls[order_idx]
            dt_matrix = dt_matrix.view(len(body_idx), len(face_idx))
            gt_matrix = rcnn_gt_cls[order_idx]
            gt_matrix = gt_matrix.view(len(body_idx), len(face_idx)).half()
            ...
            for x in range(gt_matrix.shape[1]):
                ...
                if loss_col is not None:
                    losses.append(loss_col)
        raw_loss = F.binary_cross_entropy_with_logits(pred_rcnn_cls.squeeze(1), rcnn_gt_cls.half())
        cls_loss = sum(losses) / len(losses) + raw_loss if len(losses) > 0 else raw_loss
    ...
    def cal_loss(self, input, rois1, rois2, pair_idx, pred_rcnn_cls, rcnn_gt_cls, prefix, cls_loss_scale, binary_mask=None):
        pdb.set_trace()
        loss = {}
        def f2(x):
            return Variable(torch.from_numpy(x)).cuda()
...
class GeneralAssociation(PairWiseNet):
    def __init__(self,
        ...
        self.fc_rcnn_cls_group = nn.Linear(feat_planes + self.cls_mdim, 1)
    ...
    def predict(self, x, rois1=None, rois2=None, union_rois=None, input=None):
        ...
        pred_cls = self.fc_rcnn_cls_group(feature)
        if self.tocaffe:
...
class TwoBranchAssociation(PairWiseNet):
    ...
    def get_loss(self, input):
        ...
        if isinstance(pred_rcnn_cls, list):
            loss, acc = {}, {}
            for i, pred_cls in enumerate(pred_rcnn_cls):
                cur_loss, cur_acc = self.cal_loss(input, tmp_rois1, tmp_rois2, tmp_pair_rois, pred_cls, rcnn_gt_cls, str(i) + '_', self.cfg.get("cls_loss_scale", 1.0), self.binary_mask)
                loss.update(cur_loss)
                acc.update(cur_acc)
        else:
            loss, acc = self.cal_loss(input, tmp_rois1, tmp_rois2, tmp_pair_rois, pred_rcnn_cls, rcnn_gt_cls, "", self.cfg.get("cls_loss_scale", 1.0), self.binary_mask)
        predict_vector = None
        predict_target = None
        if self.output_predict:
            ...
            rpn_loss, rpn_acc = self.cal_loss(input, tmp_rois1, tmp_rois2, pair_rois, pred_rcnn_cls, rcnn_gt_cls, "rpn_", rpn_cfg.get("cls_loss_scale", 1.0))
